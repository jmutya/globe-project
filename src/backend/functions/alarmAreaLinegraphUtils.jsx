...
/**
 * Fetches and processes area alarm data from the latest Excel file in Supabase storage.
...
export const fetchAreaAlarmData = async (forceRefresh = false) => {
  ...
  if (forceRefresh || !cachedData || !cachedTimestamp || (now - parseInt(cachedTimestamp, 10) >= CACHE_DURATION)) {
    console.log("Area alarm data cache either forced refresh, expired, or missing. Clearing cache and fetching fresh data.");
    localStorage.removeItem(CACHE_KEY);
    localStorage.removeItem(CACHE_TIMESTAMP_KEY);
  }

  // 2. Fetch fresh data
  console.log("Fetching fresh area alarm data...");
  try {
    ...
    console.log("Fresh area alarm data fetched and cached.");
