...
export const fetchAlarmCategoryChartData = async () => {
  ...
  try {
    ...
    for (const row of sheet.slice(1)) {
      let stateValue = String(row[stateIndex] || "").trim(); // Get value, convert to string, trim

      // If state value is empty, categorize it as "Unknown"
      if (!stateValue) {
        stateValue = "Unknown";
      }

      categoryCounts[stateValue] = (categoryCounts[stateValue] || 0) + 1;
      totalCount++; // Increment total count for every valid row
    }
