...
export const fetchmycomChartData = async () => {
  ...
  try {
    ...
    const assignedtoIndex = headers.indexOf("assigned_to"); // Get the index of the "assigned_to" column

    if (stateIndex === -1) {
      console.warn("Column 'state' not found in the Excel sheet headers. Cannot apply state conditions for categorization.");
      localStorage.removeItem(CACHE_KEY);
      localStorage.removeItem(CACHE_TIMESTAMP_KEY);
      localStorage.removeItem(CACHE_TOTAL_COUNT_KEY);
      return { formattedData: [], totalCount: 0 };
    ...
    // assigned_to is optional for some categorization paths, but needed for others.
    // If it's missing, it will effectively be treated as "empty" for the checks.
    if (assignedtoIndex === -1) {
      console.warn("Column 'assigned_to' not found in the Excel sheet headers. Categorization involving 'assigned_to' might be affected.");
    }

    // Initialize counts for the specific categories you want for the pie chart
    const categoryCounts = {
      "Used": 0,
      "Unused": 0,
      "Invalid": 0,
      // You might want an "Other" or "Uncategorized" if rows don't fit any rule
      // "Other": 0
    };
    let totalCount = 0; // This will count only the entries that fit the 'MYCOM Integration User' criteria

    // Step 4: Iterate through the rows (skipping header) and apply the new conditions
    for (const row of sheet.slice(1)) {
      const callerIdValue = String(row[callerIdIndex] || "").trim(); // Get caller_id value
      const stateValue = String(row[stateIndex] || "").trim(); // Get state value
      // Safely get assigned_to value, default to empty string if index is -1
      const assignedtoValue = assignedtoIndex !== -1 ? String(row[assignedtoIndex] || "").trim() : "";

      // Primary filter: Only process rows where caller_id is "MYCOM Integration User"
      if (callerIdValue === "MYCOM Integration User") {
        let assignedCategory = null; // To hold "used", "unused", or "invalid"

        // Condition 1: caller_id is "MYCOM Integration User" AND state is "cancelled"
        // (Combines your two "invalid" conditions as both depend only on state for 'cancelled')
        if (stateValue === "Cancelled") {
          assignedCategory = "Invalid";
        }
        // Condition for "used": caller_id is "MYCOM Integration User", assigned_to has value, and state is NOT "cancelled"
        else if (assignedtoValue) { // assigned_to has a value (is not empty)
            assignedCategory = "Used";
        }
        // Condition for "unused": caller_id is "MYCOM Integration User", assigned_to doesn't have value, and state is NOT "cancelled"
        else if (!assignedtoValue) { // assigned_to does NOT have a value (is empty)
            assignedCategory = "Unused";
        }

        // If a category was assigned, increment its count and the total count
        if (assignedCategory) {
          categoryCounts[assignedCategory] = (categoryCounts[assignedCategory] || 0) + 1;
          totalCount++; // Increment total count only for rows that are categorized
        }
        // else {
        //   // Handle cases that don't fit any of your defined categories but are from MYCOM Integration User
        //   // For example, if assigned_to is empty but state is not cancelled, and you don't want it as "Unused"
    ...
    // Step 5: Format the data for the chart - using the fixed categories
    const formattedData = Object.entries(categoryCounts)
      .filter(([, value]) => value > 0) // Only include categories that have a count > 0
      .map(([name, value], index) => {
        // Assign specific colors for consistency
        let fill = chartColors[index % chartColors.length]; // Fallback
    ...
    console.error("Error fetching or processing automatic ticketing chart data:", error);
